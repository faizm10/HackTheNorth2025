...
export function StudySession({
            ...
            lessons: [
              {
                ...
                examples:
                  "2×3 matrix: [[1,2,3],[4,5,6]]. Identity: [[1,0],[0,1]].",
                difficulty: "beginner",
              },
              {
  ...
  const toLinearIndex = (unitIdx: number, lessonIdx: number) =>
    courseStructure
      .slice(0, unitIdx)
      .reduce((s, u) => s + u.lessons.length, 0) + lessonIdx;
  ...
  // Allow continue only when the current lesson's module is completed
  const canContinue = lessonComplete;

  // Reset completion flag when navigating to a new lesson
  useEffect(() => {
  ...
  return (
    <Layout style={{ minHeight: "100vh", background: "#fff" }}>
      <Sider
        width={240}
        theme="light"
        style={{ padding: 16, borderRight: "1px solid #f0f0f0" }}
      >
        <Button
          onClick={onBack}
        ...
        <Text type="secondary">
          {currentUnitData.unitName} • Lesson {currentLesson + 1} of{" "}
          {currentUnitData.lessons.length}
        </Text>
        ...
        >
          ...
          <Card bodyStyle={{ padding: 12 }}>
            ...
            >
              ...
              >
                {currentUnit === courseStructure.length - 1 &&
                currentLesson === currentUnitData.lessons.length - 1
                  ? "Complete"
                  : "Continue"}
              </Button>
            </div>
          ...
          <div>
            ...
            <ChatTutor
              ...
              onModuleComplete={() => setLessonComplete(true)}
            />
            {/* removed the quiz */}
          </div>
        </div>
